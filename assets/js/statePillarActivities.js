...
// path client rect
  ...
  for (let i = 0; i < 600; i++) {
    ...
    if (elmt && elmt.className.baseVal === 'path' && elmt.id === lgaId.replace(/ +/g, '')) {
...
function appendDefs (id, number, pillarType) {
  ...
  svgContainer.append('defs').append('symbol')
    .attr('id', 'def' + id)
    .attr('class', pillarType)
    .attr('viewBox', '-10 -34 22 36').append('g').attr('fill', 'none').attr('fill-rule', 'evenodd')
    .append('path')
    .attr('stroke', 'black')
    .attr('d', 'm0, 1 l-8.8, -17.7 c-3.3, -6.6 1.4, -14.3 8.8, -14.3 l0, 0 c7.4, 0 12.1, 7.7 8.8, 14.3 l-8.8, 17.7 z ');

  const group = d3.select('#def' + id);
...
function filteredLga (target, array = []) {
  return array.find((item) => { return item.lgaName === target;});
}
...
(function () {
  ...
  async function loaded () {
    ...
    try {
      const lgaPillarPromise = await fetch(`${MMDP_BASE_URL}/api/v1/location?state=${stateName}&focusAreaName`);
      ...
      $('#state-map-pillars').load(stateUrl,
        function (responseTxt, statusTxt) {
          if (statusTxt === 'success') {
            ...
            link.addEventListener('click', () => window.location.href = `http://${baseURL}/state.html?state=${stateName}`);
          }
        });
    } catch (error) {
      window.location.href = `http://${baseURL}/state.html?state=${stateName}` || `http://${baseURL}/index-cordination-matrix.html`;
    }
  }

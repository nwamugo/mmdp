...
const createPotentialPartnershipDetailsColumn = stakeholderDetailsItem => {
  ...
  return `
    <div class="org-potential-partnership">
        <span class="organization-title-partnerships-modal"> ${
          stakeholderDetailsItem.organizationName
        }</span>
          <div class="org-text-container-potential-partnerships">
          <ul class="org-details-list">
            <li><b>Amount Invested:</b> <span>NGN ${thousandSeparators(
              stakeholderDetailsItem.totalAmountInvested
            ) || '0'}</span> 
            <span><b><i>(${stakeholderDetailsItem.amountInvestedPercentage ||
              '00%'} of Total in LGA)</i></b></span></li>
            <li><b>Beneficiaries Reached:</b> <span> ${thousandSeparators(
              stakeholderDetailsItem.totalBeneficiariesReached
            ) || '0'}</span>
            <span><b><i>(${stakeholderDetailsItem.beneficiariesPercentage ||
              '00%'} of Total in LGA)</i></b></span></li>
            <li><b>Staff Strength:</b> <span>${stakeholderDetailsItem.staffStrength ||
              'Unavailable'}</span></li>
          </ul>
          </div>
    </div>
...
const bindPotentialPartnershipModalJQuery = potentialPartnershipsByFocusArea => {
  ...
  $('.pprow-item').click(async function() {
    ...
    const currentTableRowModalData = potentialPartnershipsByFocusArea.find(
      ppTableRow => ppTableRow.ppRowId === $(this).attr('pprowid')
    );
    ...
    for (
      let stakeholderItemIndex = 0;
      stakeholderItemIndex < organizationDetailsArray.length;
      stakeholderItemIndex++
    ) {
      const currentStakeholderDetails =
        organizationDetailsArray[stakeholderItemIndex];
      organizationDetailsHTML += createPotentialPartnershipDetailsColumn({
        ...
        amountInvestedPercentage:
          currentStakeholderDetails.amountInvestedPercentage,
        totalBeneficiariesReached:
          currentStakeholderDetails.totalBeneficiariesReached,
        beneficiariesPercentage:
          currentStakeholderDetails.beneficiariesPercentage,
    ...
    const output = `
            <div class="lga-grid-box">
              <div class="lga-header">
                <div class="lga-name">
                  <p id="lgaName">${currentTableRowModalData.lgaName}</p>
                </div>
                <div class="lga-stats">
                  <p>Total number of beneficiaries: ${thousandSeparators(
                    currentTableRowModalData.totalNumberOfBeneficiariesReached
                  )}</p>
                  <p>Total number of stakeholders: ${thousandSeparators(
                    currentTableRowModalData.totalNumberOfStakeholders
                  )}</p>
                  <p>Total number of returnees: ${thousandSeparators(
                    currentTableRowModalData.totalNumberOfReturnees
                  )}</p>
                </div>
              </div>
              <div class="lga-body">
    ...
    document.querySelector(
      '.modal-grid-container-partnerships'
    ).innerHTML = output;

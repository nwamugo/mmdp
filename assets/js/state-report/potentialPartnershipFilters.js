...
function hasPartnershipWithCurrentStakeholders(currentStakeholderId, compareStakeholderId, focusArea, allCurrentPartnershipPairsArray){
  if(allCurrentPartnershipPairsArray){
    for ( let currentPartnership = 0; currentPartnership < allCurrentPartnershipPairsArray.length ; currentPartnership++){
      ...
      if  ((currentStakeholderId === stakeholder1Id || currentStakeholderId === stakeholder2Id)
        ...
        return { 'hasPartnership':true, 'partnerkeys1': (focusArea+currentStakeholderId+compareStakeholderId), 'partnerkeys2' : (focusArea+compareStakeholderId+currentStakeholderId) };
      }
      if  (currentStakeholderId === stakeholder1Id || currentStakeholderId === stakeholder2Id){
        return { 'hasPartnership':false, 'partnerkeys1': (focusArea+currentStakeholderId+compareStakeholderId), 'partnerkeys2' : (focusArea+compareStakeholderId+currentStakeholderId) };
      }
    }
  }
  return { 'hasPartnership':false, 'partnerkeys1': (focusArea+currentStakeholderId+compareStakeholderId), 'partnerkeys2' : (focusArea+compareStakeholderId+currentStakeholderId) }
      ...
      if (currentStakeholder.LGA === currentLGA['lgaName']) {
        ...
        for (let item2 = 0; item2 < currentLGA['focusAreas'].length; item2++) {
          ...
          if (currentStakeholder.focusArea === currentFocusArea.name) { // current stakeholder focus area
            ...
            for (let currentBeneficiaryIndex = 0; currentBeneficiaryIndex < arr.length; currentBeneficiaryIndex++) {
              const currentPartnershipKey1 = (currentFocusArea.name + currentStakeholder.stakeholderId + arr[currentBeneficiaryIndex].stakeholderId);
              const currentPartnershipKey2 = (currentFocusArea.name + arr[currentBeneficiaryIndex].stakeholderId + currentStakeholder.stakeholderId);
              if (
                ...
                if (arr[currentBeneficiaryIndex].focusArea === currentFocusArea.name
                    && arr[currentBeneficiaryIndex].LGA === currentStakeholder.LGA && currentStakeholder.stakeholderId !==
                  ...
                  if (!filteredPartnerships || filteredPartnerships.length === 0) {
                    ...
                    partnershipMap.set(
                        (currentFocusArea.name+currentStakeholder.stakeholderId + arr[currentBeneficiaryIndex].stakeholderId),
                        true);
                    partnershipMap.set(
                        (currentFocusArea.name+arr[currentBeneficiaryIndex].stakeholderId + currentStakeholder.stakeholderId),
                        true);
                  }
                  ...
                  if (!partnershipMap.has(partnershipStatus.partnerkeys1)

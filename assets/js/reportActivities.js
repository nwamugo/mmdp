...
function getPoint(x, y) {
  const svg = document.querySelector("#report-map").querySelector("svg");
...
function appendDefs(id, number, pillarType) {
  const svgContainer = d3.select("#report-map").select("svg");
...
(function() {
  ...
  async function loaded() {
    ...
    try {
      ...
      $("#report-map").load(stateUrl, function(responseTxt, statusTxt) {
        if (statusTxt === "success") {
          const [, xmlPart, svgPart] = responseTxt.match(
            /([\s\S.]*)(<svg[\s\S]*<\/svg>)/
          );
          $("#report-map").html(svgPart);

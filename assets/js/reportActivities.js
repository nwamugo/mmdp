...
const queryNameFromUrl = window.location.search
    .substring(1)
    .split('=')[1];
const queryParam = queryNameFromUrl
    ? queryNameFromUrl.charAt(0).toUpperCase() +
queryNameFromUrl.slice(1)
    : 'Nigeria';
...
async function fetchStakeholderInformation(){
    ...
    for (const item of window.data.filteredStakeholders) {
        for (const beneficiary of item.beneficiaries) {
            ...
            impactFactorRow.focusArea = beneficiary.focusArea.focusAreaName.focusAreaName;
            ...
            impactFactorRow.subtheme = beneficiary.focusArea.subThemeName.subThemeName;
            impactFactorRow.pillar = beneficiary.focusArea.thematicPillarName.pillarName;
            ...
            if ( focusAreaTarget > totalBeneficiaries ) {
                impactFactorRow.targetCompletion = ( totalBeneficiaries/focusAreaTarget ) * 100 + '%';
            } else {
